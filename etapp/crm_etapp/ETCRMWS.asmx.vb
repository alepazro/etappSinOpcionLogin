...
Public Class ETCRMWS
   ...
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    ...
    <WebMethod()>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function crmGetDeviceDataSendMSM() As String
        Dim strResult As String = ""
        Dim token As String = ""
        Dim did As String = ""
        Dim dl As New crmDataLayer
        ...
        Try
            ...
            If IsGUID(token) Then
                strResult = dl.CRM_HDevices_GetSendSMS(token, did)
            End If
        ...
        Catch ex As Exception
            BLErrorHandling.ErrorCapture(pSysModule, "crmGetDeviceDataSendMSM", "", ex.Message, 0)
        End Try
    ...
    <WebMethod()>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function crmGetUrl() As String
        Dim url As String = ""
        Dim token As String = ""
        Try
            If Not IsNothing(HttpContext.Current.Request.Form("t")) Then
        ...
        Catch ex As Exception
            BLErrorHandling.ErrorCapture(pSysModule, "crmGetUrl", "", ex.Message, 0)
        End Try
