...
Public Interface Icrm
    ...
    <OperationContract()>
    <WebInvoke(Method:="GET", BodyStyle:=WebMessageBodyStyle.Bare, RequestFormat:=WebMessageFormat.Json, ResponseFormat:=WebMessageFormat.Json, UriTemplate:="getdevicesAccelerometer?token={token}&companyid={companyid}")>
    Function getdevicesAccelerometer(ByVal token As String, companyid As Integer) As List(Of DevicesAccelerometer)
    <OperationContract()>
    <WebInvoke(Method:="GET", BodyStyle:=WebMessageBodyStyle.Bare, RequestFormat:=WebMessageFormat.Json, ResponseFormat:=WebMessageFormat.Json, UriTemplate:="getAccelConfiguration?token={token}")>
    Function Devices_AccelConfiguration(ByVal token As String) As List(Of Devices_AccelConfiguration)

    <OperationContract()>
    <WebInvoke(Method:="POST", BodyStyle:=WebMessageBodyStyle.Bare, RequestFormat:=WebMessageFormat.Json, ResponseFormat:=WebMessageFormat.Json, UriTemplate:="putaccelerometer/{token}")>
    Function putDevices_AccelConfiguration(ByVal token As String, ByVal PutAceleration As PutAceleration) As responseOk
