...
function dbReadWriteWS(ws, methodName, data, alertFailure, isAsync) {
                ...
                success: function (xml, textStatus) {
                    ...
                    if (textStatus == 'success') {
                        ...
                        //else {
                            ...
                            jsonObj = $.parseJSON(xml);// eval('(' + $("string", xml).text() + ')');
...
function postDb(method, data, params) {
    try {
        var result = false;
        var url = 'https://pre.etrack.ws/ws.svc/' + method;
        // var url = 'http://localhost:85/ws.svc/' + method;
