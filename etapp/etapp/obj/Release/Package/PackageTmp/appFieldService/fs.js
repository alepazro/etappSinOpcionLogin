...
function saveJob(via) {
    try {
        ...
        if (validateInformation()) {
            ...
            let jobstatus = $("#jobstatus option:selected").val();
...
function JobSupport() {
    try {
        ...
        $.each(jobs.statuses, function (i, status) {
...
function clearJobs(source) {
    ...
    if (source == 2) {
        ...
        $("#jobpriority option[value='-1']").attr("selected", true);
        $("#jobcategories option[value='-1']").attr("selected", true);
        $("#jobstatus option[value='-1']").attr("selected", true);        
        $("#ListDrivers option[value='-1']").attr("selected", true);        
        $("#date2").val('');
        $("#date1").val('');
        $("#dispatchJobDescription").val('');
...
function loadJobNew(id) {
    try {
        ...
        if (id != '') {
            ...
            $("#jobstatus option[value=" + Job.StatusID + "]").attr("selected", true);
...
function existingStop() {
    ...
    document.getElementById('stopexisting').checked = false;
    win.focus();
    /*loadstopexisting();*/
}
...
function loadstopexisting(source) {
    ...
    opt0TXT = document.createTextNode("Select Stop..");
