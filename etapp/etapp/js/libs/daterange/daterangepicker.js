...
!function ($) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        var DRPTemplate = '<div class="' + options.dateRangePickerID + ' daterangepicker dropdown-menu" >' + 
            '<table class="AllElement">'+
                
             //   '<div class="calendar left" style="float:left;width:270px;"></div>' +
             //   '<div class="calendar right" style="float:right;width:270px;"></div>' +
                '<tr><td style="text-align:left; vertical-align:top">' +
                    '<table><tr><td>' +
                        ...
                        '</div></td></tr>' +
                        '<tr><td>' +
                            '<button class="applyBtn"  id="applyBtn"></button>&nbsp;' +
                            '<button class="cancelBtn" id="cancelBtn"></button>' +
                        '</td></tr></table>' +                    
                    '</td>' +
                    '<td style="vertical-align:top"><div class="calendar right" style="width:270px;"></div></td>' +
                    '<td style="vertical-align:top"><div class="calendar left" style="width:270px;"></div></td>' +
                '</tr></table>' +
              '</div>';

        this.parentEl = (typeof options == 'object' && options.parentEl && $(options.parentEl)) || $(this.parentEl);

        $(this.parentEl).find('.' + options.dateRangePickerID).remove();
        ...
        this.container.find('.AllElement')
        .on('click', '.applyBtn', function () {
            $.proxy(this.clickApply, this);
            $.proxy(this.updateFormInputs, this)
            $('.imgDdl').trigger('apply', this);         
            $('.' + options.dateRangePickerID).hide();

            $('body').click();
        })
        .on('click', '.cancelBtn', $.proxy(this.clickCancel, this))
    ...
    DateRangePicker.prototype = {
          ...
          //  console.log("parent id-->"+this.parent);
            ...
            if (typeof options.ranges == 'object') {
                ...
                for (var range in this.ranges) {
                    list += '<li data-code="' + options.ranges[range][2] + '">' + range + '</li>';
                }
                if(options.enableCustom == true)
                    list += '<li data-code="custom">' + this.locale.customRangeLabel + '</li>';
                list += '</ul>';
        ...
        updateFormInputs: function () {
            ...
            //display = '<b>' + app.language.getLabel('DateFilter.Custom') + '</b>: ' + '@From - @To ';
            display =  '@From - @To ';

            display = display.replace('@From', this.startDate.format('YYYY-MM-DD HH:mm'));
            display = display.replace('@To', this.endDate.format('YYYY-MM-DD HH:mm'));
            $(thatDate).find('span').css("font-size", "7px;");

           // console.log("display -->" + display);
            $(thatDate).find('span').html(display);

            $(thatDate).attr('data-from', this.startDate.format('YYYY-MM-DD HH:mm:ss'));
            $(thatDate).attr('data-to', this.endDate.format('YYYY-MM-DD HH:mm:ss'));
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
            ...
            html += '<table class="table-condensed1">';
            html += '<thead>';
            html += '<tr>';
