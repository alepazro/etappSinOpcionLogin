...
function editGeofence(obj) {
    try {
        ...
        var geo = getGeofence(id);
        if (geo.geofence.length > 0) {
            var jsonItem = eval('(' + geo.geofence[0] + ')');

            originalName = jsonItem.name;

            $('#newGeofenceName').val(jsonItem.name);
            $('#newGeofenceName').attr('data-id', id);
            $('#newGeofenceAddress').val(jsonItem.fullAddress);
            ...
            if (jsonItem.contactSMSAlert == true) {
            ...
            if (jsonItem.contactEmailAlert == true) {
            ...
            $('#geoContactAlertType').val(jsonItem.contactAlertTypeId);

            $('#cbxContAlertArr').val(jsonItem.arrivalMsgId);
            if (jsonItem.arrivalMsgId == 0) {
            ...
            $('#cbxContAlertDep').val(jsonItem.departureMsgId);
            if (jsonItem.departureMsgId == 0) {
            ...
            if (jsonItem.isSpeedLimit == true) {
                $('#newGeofenceIsSpeedLimit').prop('checked', true);
            }
            else {
                $('#newGeofenceIsSpeedLimit').prop('checked', false);
            }
            $('#newGeofenceSpeedLimit').val(jsonItem.speedLimit);

            $('#cbxGeofencesTypes').val(jsonItem.geofenceTypeId);
            $('#newGeofenceRadius').val(jsonItem.radius);
            $('#newGeofenceComments').val(jsonItem.comments);
            $('#cbxGeofencesAlertsTypes').val(jsonItem.geofenceAlertTypeId);

            var shapeId = jsonItem.shapeId;
            initGeoType(shapeId);
            ...
            $(divResult).attr('data-latitude', jsonItem.latitude);
            $(divResult).attr('data-longitude', jsonItem.longitude);
            $(divResult).attr('data-fullAddress', jsonItem.fullAddress);

            $(divResult).attr('data-contactName', jsonItem.contactName);
            $(divResult).attr('data-phone', jsonItem.phone);

            //GEOFENCE CONTACT NOTIFICATIONS. 11/23/2013
            $(divResult).attr('data-contactEmail', jsonItem.contactEmail);
            $(divResult).attr('data-contactHasSMSAlert', jsonItem.contactSMSAlert);
            $(divResult).attr('data-contactHasEmailAlert', jsonItem.contactEmailAlert);
            $(divResult).attr('data-contactAlertTypeId', jsonItem.contactAlertTypeId);
            $(divResult).attr('data-arrivalMsgId', jsonItem.arrivalMsgId);
            $(divResult).attr('data-departureMsgId', jsonItem.departureMsgId);
            //============================================

            $(divResult).attr('data-isSpeedLimit', jsonItem.isSpeedLimit);
            $(divResult).attr('data-speedLimit', jsonItem.speedLimit);

            $(divResult).attr('data-postalCode', jsonItem.postalCode);
            $(divResult).attr('data-country', jsonItem.country);
            $(divResult).attr('data-state', jsonItem.state);
            $(divResult).attr('data-county', jsonItem.county);
            $(divResult).attr('data-city', jsonItem.city);
            $(divResult).attr('data-street', jsonItem.street);
            $(divResult).attr('data-route', jsonItem.route);
            $(divResult).attr('data-streetNumber', jsonItem.streetNumber);
            $(divResult).attr('data-suite', jsonItem.suite);
            $(divResult).attr('data-shapeId', jsonItem.shapeId);
            $(divResult).attr('data-jsonPolyVerticesTXT', jsonItem.jsonPolyVerticesTXT);
            div.appendChild(divResult);

            var spanResult = document.createElement('span');
            $(spanResult).text(jsonItem.fullAddress);
...
function saveNewGeofence() {
    ...
    try {
        ...
        if (isOk == true) {
            ...
            //var url = 'https://localhost:44385/etrack.svc/saveGeofence';
...
function drawPoly(jsonVertices) {
    try {
        ...
        for (var i = 0; i < jsonVertices.length; i++) {
            ...
            //var marker = new google.maps.Marker({ map: newGeofenceMap, position: loc});
        ...
        poly.setMap(null);
        poly = new google.maps.Polygon({ map: newGeofenceMap, path: path, strokeColor: "#FF0000", strokeOpacity: 0.8, strokeWeight: 2, fillColor: "#FF0000", fillOpacity: 0.35 , editable: true});
    }
    catch (err) {
    }
}
...
function changeIcon() {
    ...
    try {
        var icon = $("#cbxGeofencesTypes option:selected").attr("data-icon")
        //var id = $("#cbxGeofencesTypes").val();
        //var icon = document.getElementById("cbxGeofencesTypes").item(id).getAttribute("data-id");
        $("#imgGeoIcons").attr("src", "");
        $("#imgGeoIcons").attr("src", icon);
    } catch (error) {
        $("#imgGeoIcons").attr("src", "");
    }
